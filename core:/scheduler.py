...
class AutomationPipeline:
    ...
    def run_daily_pipeline(self, progress_callback=None) -> dict:
        ...
        try:
            ...
            niche = cfg.get("channel_niche", "Technology")
            voice_name = cfg.get("voice_name", "en-US-GuyNeural")
            image_style = cfg.get("image_style", "Cinematic")
            num_images = cfg.get("images_per_video", 110)
            target_words = cfg.get("target_word_count", 1550)

            # 1. Topic
            if progress_callback: progress_callback("Step 1/7: Researching Viral Topic...", 10)
            topic_info = TopicResearcher(api_key).find_trending_topic(niche)
            log_data["steps"]["topic"] = topic_info

            # 2. Script
            if progress_callback: progress_callback("Step 2/7: Writing 1500+ Word Script...", 25)
            script_data = ScriptWriter(api_key).generate_full_script(topic_info, target_words, num_images)
            scenes = script_data.get("scenes", [])

            # 3. TTS
            if progress_callback: progress_callback("Step 3/7: Generating Voice-Over Audio...", 40)
            ...
            # 4. Images
            if progress_callback: progress_callback(f"Step 4/7: Generating {len(scenes)} Visual Images...", 60)
            img_paths = ImageGenerator(image_style).batch_generate_images(scenes, images_dir, style=image_style)
