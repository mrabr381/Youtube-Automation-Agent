...
class VideoEditor:
    ...
    def _render_single_scene(self, img_path: str, aud_path: str, out_scene_path: str, zoom_in: bool = True) -> bool:
        """
        Ultra-sharp 1080p Ken Burns zoom animation without artificial blur.
        """
        if zoom_in:
            zoom_filter = f"zoompan=z='min(zoom+0.0015,1.18)':d=125:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={self.width}x{self.height}:fps={self.fps}"
        else:
            zoom_filter = f"zoompan=z='if(lte(zoom,1.0),1.18,max(1.0,zoom-0.0015))':d=125:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={self.width}x{self.height}:fps={self.fps}"
        ...
        try:
            res = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=75)
    ...
    def assemble_video(self, scenes_data: list, image_paths: list, audio_paths: list, output_video_path: Path) -> str:
        ...
        print(f"[VideoEditor] Rendering {total_scenes} scenes individually for crystal-clear 1080p...")
